...
public class ListQuery {
  ...
  public static List<Row> list(
    ...
    final Column[] columns = wantedColumns.stream().map(functions::col).toArray(Column[]::new);
    dataframe = zipWithIndex(dataframe.filter(condition.getCondition()));
  ...
  private static Dataset<Row> zipWithIndex(Dataset<Row> df) {
    Dataset<Row> dfWithPartitionId =
        df.withColumn("partition_id", functions.spark_partition_id())
    ...
    Object partitionOffsetsObject =
        dfWithPartitionId
            ...
            .agg(
                functions.count(functions.lit(1)).alias("cnt"),
                functions.first("inc_id").alias("inc_id"))
            .orderBy("partition_id")
            .select(
                functions.col("partition_id"),
                functions
                    .sum("cnt")
                    .over(Window.orderBy("partition_id"))
                    .minus(functions.col("cnt"))
                    .minus(functions.col("inc_id"))
                    .alias("cnt"))
            .collect();
