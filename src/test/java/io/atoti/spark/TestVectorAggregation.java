...
public class TestVectorAggregation {
  ...
  void quantile() {
	  ...
	  var rows = AggregateQuery.aggregate(dataframe, List.of("id"), List.of(price_simulations), List.of(quantile)).collectAsList();
	    ...
	    for (int i = 0; i < 3; i++) { 
	    assertThat(convertScalaArrayToArray((ArraySeq<Long>) rowsById.get(1).getAs("price_simulations_sum")).get(i)).isEqualTo(List.of(3, 7, 5).get(i));
	    assertThat(convertScalaArrayToArray((ArraySeq<Long>) rowsById.get(2).getAs("price_simulations_sum")).get(i)).isEqualTo(List.of(1, 3, 2).get(i));
	    }
  }
  ...
  void vectorAt() {
	  ...
	  var rows = AggregateQuery.aggregate(dataframe, List.of("id"), List.of(), List.of(vectorAt)).collectAsList();
...
@Test
  void simpleAggregation() {
    ...
    var rows = AggregateQuery.aggregate(
            dataframe, List.of("id"), List.of(sumVector)).collectAsList();
 ...
 // result must have 2 values
 	  ...
 	  final var rowsById =
 		        rows.stream()
                     ...
                     for (int i = 0; i < 3; i++) {
                         assertThat(convertScalaArrayToArray((ArraySeq<Long>) rowsById.get(1).getAs("sum(vector)")).get(i)).isEqualTo(List.of(3L, 7L, 5L).get(i));
                         assertThat(convertScalaArrayToArray((ArraySeq<Long>) rowsById.get(2).getAs("sum(vector)")).get(i)).isEqualTo(List.of(1L, 3L, 2L).get(i));
                     }
  }
...
@Test
 ...
 // result must have 2 values
	  ...
	  for (int i = 0; i < 3; i++) {
	  assertThat(convertScalaArrayToArray((ArraySeq<Long>) rowsById.get(1).getAs("f * vector")).get(i)).isEqualTo(List.of(15L, 35L, 25L).get(i));
	    assertThat(convertScalaArrayToArray((ArraySeq<Long>) rowsById.get(2).getAs("f * vector")).get(i)).isEqualTo(List.of(2L, 6L, 4L).get(i));
	  }
  }
 ...
 // result must have 3 values
  ...
  void simulationExplorationAtQuantile() {
    ...
    final var revenues =
        new Multiply(
            "f * vector", new Sum("f", "factor-field"), new SumArray("sum(vector)", "vector-field", spark.implicits().newLongArrayEncoder()));
    final List<Row> rows =
    ...
    var result = AggregateQuery.aggregate(
            dataframe,
            List.of("simulation"),
            List.of(),
            List.of(
